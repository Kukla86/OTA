// Клиент Amadeus: OAuth и поиск
import { SearchQuery, FlightOffer } from '@/lib/types'
...
export function normalizeAmadeus(data: any): FlightOffer[] {
  // Упрощенная нормализация под базовый тип
  const offers: FlightOffer[] = (data.data || []).map((o: any, idx: number) => {
